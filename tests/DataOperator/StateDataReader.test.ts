import { IStateData } from "../../src/DataOperator/StateDataOperator/common";
import { StateDataReader } from "../../src/DataOperator/StateDataOperator/StateDataReader";
import { StateDataWriter } from "../../src/DataOperator/StateDataOperator/StateDataWriter";

test('converts from state to file and back', () => {
  const originalState: IStateData[] = [
    {
      id: 1,
      reward: 100,
  ...
  StateDataWriter.writeStateData(originalState, filePath);
  const actualState = StateDataReader.readStateData(filePath);
